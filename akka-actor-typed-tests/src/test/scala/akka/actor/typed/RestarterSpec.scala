...
class RestarterSpec extends TypedSpec {
  ...
  "A restarter" must {
    ...
    "must stop when no supervise" in {
      val inbox = Inbox[Event]("evt")
    ...
    "must stop when unhandled exception" in {
      val inbox = Inbox[Event]("evt")
    ...
    "must restart when handled exception" in {
      val inbox = Inbox[Event]("evt")
      val behv = supervise(target(inbox.ref)).onFailure[Exc1](SupervisorStrategy.restart)
      val ctx = mkCtx(behv)
      ctx.run(NextState)
    ...
    "must resume when handled exception" in {
      val inbox = Inbox[Event]("evt")
    ...
    "must support nesting to handle different exceptions" in {
      val inbox = Inbox[Event]("evt")
      val behv =
        supervise(
          supervise(
    ...
    "must stop after restart retries limit" in {
      val inbox = Inbox[Event]("evt")
    ...
    "must reset retry limit after withinTimeRange" in {
      val inbox = Inbox[Event]("evt")
    ...
    "must stop at first exception when restart retries limit is 0" in {
      val inbox = Inbox[Event]("evt")
...
class RestarterStubbedSpec extends TypedSpec with StartSupport {
  ...
  "A restart (subbed)" must {
    ...
    "must NOT stop children when restarting" in {
      val probe = TestProbe[Event]("evt")
      val behv = supervise(target(probe.ref)).onFailure[Exc1](SupervisorStrategy.restart)
      val ref = start(behv)
    ...
    "must restart after exponential backoff" in {
      val probe = TestProbe[Event]("evt")
    ...
    "must reset exponential backoff count after reset timeout" in {
      val probe = TestProbe[Event]("evt")
      val minBackoff = 1.seconds
      val strategy = SupervisorStrategy.restartWithBackoff(minBackoff, 10.seconds, 0.0)
    ...
    "must stop when exception from MutableBehavior constructor" in {
      val probe = TestProbe[Event]("evt")
